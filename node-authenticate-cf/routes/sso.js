...
const sp = ServiceProvider({
  metadata: fs.readFileSync('./metadata/saml-btpTrial-sp-metadata.xml')
});
const idp = IdentityProvider({
  metadata: fs.readFileSync('./metadata/saml-oneLogin-idp-metadata.xml')
});

// Release the sp-metadata publicly
router.get('/sp-metadata', (req, res) => res.header('Content-Type', 'text/xml').send(sp.getMetadata()));
  ...
  sp.parseLoginResponse(idp, 'post', req)
    .then(parseResult => {
      ...
      if (currentDate > notBeforeDate && currentDate < notAfterDate) {
        console.log("https://approuter-node-testing-cf-boisterous-pangolin-fq.cfapps.eu10.hana.ondemand.com/node-testing/welcome-gonzales");
        res.redirect("https://approuter-node-testing-cf-boisterous-pangolin-fq.cfapps.eu10.hana.ondemand.com/node-testing/welcome-gonzales");
      }
      else {
        console.log("Nayyy");
